...
The basic idea is that instead of providing a string to `text`, `xlab` or other function through which you want to plot some text, you provide an `expression`, in the sense of a mathematical expression. Two examples are given below:
```{r}
...
text(5, 2.5, labels=expression("Amplitude (" * mu * "V)"))
box()
```
...
the second argument to the function is a list of all the variable values that need to be substituted. In the example below two values are substitited:
```{r}
plot(x,y)
